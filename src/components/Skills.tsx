  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-6">
        ...
        <h3 className="text-xl font-bold text-white group-hover:text-red-100 transition-colors duration-500 ease-out">
          {category.title}
        </h3>
      </div>

      {/* Skills grid with conditional bold */}
      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
        {category.skills.map((skill, skillIndex) => (
          <div key={skillIndex} className="flex items-center gap-2">
            ...
            <span className="text-gray-300 text-sm group-hover:text-gray-200 transition-colors duration-500">
              {skill.bold ? <strong>{skill.name}</strong> : skill.name}
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <section id="skills" className="py-20 bg-transparent">
      <div className="container mx-auto px-6">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">STEM Skills We Teach</h2>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            From programming to manufacturing, we provide comprehensive training across all areas of STEM to
            prepare you for real-world engineering challenges.
          </p>
        </div>
